...
# additional code by shuber
# we don't need all those frequencies for the kalimba
frequencies =
  'A4': 440, 'A5': 880, 'A6': 1760,
  'A#4': 466.164, 'A#5': 932.328, 'A#6': 1864.66,
  'B4': 493.883, 'B5': 987.767, 'B6': 1975.53,
  'C3': 130.813, 'C4': 261.626, 'C5': 523.251, 'C6': 1046.50,
  'C#3': 138.591, 'C#4': 277.183, 'C#5': 554.365, 'C#6': 1108.73,
  'D3': 146.832, 'D4': 293.665, 'D5': 587.330,
  'D#3': 155.563, 'D#4': 311.127, 'D#5': 622.254,
  'E3': 164.814, 'E4': 329.628, 'E5': 659.255,
  'F3': 174.614, 'F4': 349.228, 'F5': 698.456,
  'F#3': 184.997, 'F#4': 369.994, 'F#5': 739.989,
  'G3': 195.998, 'G4': 391.995, 'G5': 783.991,
  'G#3': 207.652, 'G#4': 415.305, 'G#5': 830.609
...
window.Tuner = ->
  ...
  success = (stream) ->
    ...
    display = 
      draw: (note, diff) ->
        # additional code by shuber
        # console.log [note, diff]
        $('.tuner-wrapper .note').css('visibility', 'visible').find('span').html(note)
        tine = $(".tine[data-note='#{note}']").addClass('tuned').data('tuned-at', new Date().getTime())
        duration = 1000
        callback = -> tine.removeClass('tuned') if tine.data('tuned-at') + duration < new Date().getTime()
