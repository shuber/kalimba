...
# First there is a check for the AudioContext constructor,
# which is only available in Google Chrome. This is the
# interface to the Web Audio API, which enables the majority
# of the functionality required for the tuner.
window.AudioContext = (->
...
# Similarly, there is a check for the GetUserMedia function,
# which allows the browser to have access to the audio/video
# input hardware of the device. Currently, there is only limited
# support (Chrome & Opera), but more is likely to follow.
...
class window.Tuner
  ...
  success: (@stream) =>
    ...
    try
      ...
      @interval = setInterval @process, 1000 / @processesPerSecond
    catch error
      @error error

  start: (@processesPerSecond = 10, @callback) =>
    navigator.getUserMedia audio: true, @success, @error unless @interval?
